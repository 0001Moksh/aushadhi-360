...
<body>
  ...
  const filteredBills = useMemo(() => {
    ...
    if (groupByCustomer) {
      const grouped: Record<string, BillHistory[]> = {}
      sorted.forEach(bill => {
        const key = bill.customerEmail || "Walk-in"
        if (!grouped[key]) {
          grouped[key] = []
        }
    ...
    return sorted
  }, [bills, search, quickFilter, groupByCustomer])
    ...
    <DashboardLayout>
      ...
      <Card className="p-3 mb-4 border-accent">
        ...
        <div className="flex items-center gap-2 mt-3">
          ...
          />
          <Label htmlFor="group-by-customer" className="text-sm font-normal cursor-pointer">
            Group by customer
          </Label>
        </div>
        ...
        <div className="text-sm text-foreground py-2 ml-1">
          Showing <b>{groupByCustomer ? Object.keys(filteredBills as any).length + " customers" : (filteredBills as BillHistory[]).length + " bills"}</b> {!groupByCustomer && `of ${bills.length} bills`}
        </div>

      {/* Bills */}
      {(groupByCustomer ? Object.keys(filteredBills as Record<string, BillHistory[]>).length : (filteredBills as BillHistory[]).length) === 0 ? (
        ...
        <div className="space-y-6">
          {Object.entries(filteredBills as Record<string, BillHistory[]>).map(([customer, customerBills]) => {
            const totalAmount = customerBills.reduce((sum, bill) => sum + bill.total, 0)
              ...
              <Card key={customer} className="p-4">
                <div className="flex items-center justify-between mb-4 pb-3 border-b">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {totalOrders} {totalOrders === 1 ? "order" : "orders"} • Total: ₹{totalAmount.toFixed(2)}
                    </p>
                  </div>
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {customerBills.map((bill) => (
                    <Card key={bill.id} className="p-3 hover:shadow-lg hover:border-accent transition bg-muted/30">
                      <div className="flex justify-between">
                        ...
                        <Badge variant="outline" className="h-fit">{bill.itemCount} items</Badge>
                      </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
          {(filteredBills as BillHistory[]).map((bill) => (
            <Card key={bill.id} className="p-3 hover:shadow-lg hover:border-accent transition">
              ...
              {bill.customerEmail && (
                <p className="text-xs text-muted-foreground truncate mb-2 mt-1">
                  {bill.customerEmail}
                </p>
              )}

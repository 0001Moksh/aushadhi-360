...
export function DashboardHome() {
  ...
  const { data: profile, isLoading: profileLoading } = useCachedData<UserProfile>(
    async () => {
    ...
    { key: `dashboard_profile_${email}`, staleTime: 2 * 60 * 1000 } // 2 min fresh
  )

  const { data: medicines, isLoading: medicinesLoading } = useCachedData(
    async () => {
      if (!email) throw new Error("No email found")
    ...
    { key: `dashboard_medicines_${email}`, staleTime: 3 * 60 * 1000 } // 3 min fresh
  )

  const { data: billsData, isLoading: billsLoading } = useCachedData(
    async () => {
      if (!email) throw new Error("No email found")
      const res = await fetch(`/api/billing/history?email=${encodeURIComponent(email)}&limit=200`)
      if (!res.ok) throw new Error("Failed to fetch bills")
      return res.json()
    },
    { key: `dashboard_bills_${email}`, staleTime: 5 * 60 * 1000 } // 5 min fresh
  )
  ...
  // Process medicines data when loaded
  useEffect(() => {
    if (!medicines) return
  ...
  // Process bills data when loaded
  useEffect(() => {
    if (!billsData) return
  ...
  const isLoading = profileLoading || medicinesLoading || billsLoading
